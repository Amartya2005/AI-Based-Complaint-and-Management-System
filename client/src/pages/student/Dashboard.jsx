    ...
    return (
        ...
        >
            <div
                ...
                style={{ transform: transform ? transform : 'perspective(1000px) rotateX(0deg) rotateY(0deg) scale3d(1, 1, 1)', transition: transform ? 'transform 0.1s ease-out' : 'transform 0.5s ease-out' }}
            ...
            >
                <div className={`p-6 flex-1 relative z-10 ${stat.textClass || 'text-white'}`}>
                    <h3 className="text-4xl font-black mb-1 drop-shadow-sm">{stat.value}</h3>
                ...
                {/* Bottom link strip */}
                <Link to="/student/my-complaints" className="bg-black/10 backdrop-blur-sm hover:bg-black/20 transition-colors py-2.5 px-6 flex justify-between items-center text-sm relative z-10">
                    <span className={`${stat.textClass || 'text-white'} font-medium`}>View details</span>
                    <div className={`w-6 h-6 rounded-full bg-white/20 flex items-center justify-center ${stat.textClass || 'text-white'} group-hover:translate-x-1 transition-transform`}>
                        <ArrowRight size={14} />
                    </div>
                </Link>
            </div>
...
const StudentDashboard = () => {
    ...
    const [formData, setFormData] = useState({ title: '', category: 'HOSTEL', description: '' });
    ...
    const admin = complaints.filter(c => c.category === 'ADMINISTRATIVE').length;
    ...
    const handleNewComplaintSubmit = async (e) => {
        ...
        try {
            ...
            addToast(typeof detail === 'string' ? detail : 'Failed to submit complaint.', 'error');
        } finally {
    ...
    const stats = [
        { label: 'Total Filed', value: total, bg: 'bg-gradient-to-br from-[#17a2b8] to-[#128293]', shadow: 'shadow-[#17a2b8]/40', icon: Activity },
        { label: 'Resolved', value: resolved, bg: 'bg-gradient-to-br from-[#28a745] to-[#1e7e34]', shadow: 'shadow-[#28a745]/40', icon: CheckCircle },
        { label: 'In Progress', value: inProgress + assigned, bg: 'bg-gradient-to-br from-[#8ba1b6] to-[#607182]', shadow: 'shadow-[#8ba1b6]/40', icon: Clock },
        ...
        >
            ...
            <motion.div variants={itemVariants} className="bg-white/80 backdrop-blur-md rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100/80 overflow-hidden relative">
                ...
                <div className="p-5 border-b border-gray-100/80 flex items-center gap-3">
                    ...
                    <h3 className="text-[#2c323f] font-bold text-base tracking-wide">Category Summary</h3>
                </div>
